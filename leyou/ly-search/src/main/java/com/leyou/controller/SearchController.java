...
public class SearchController {
    ...
    @RequestMapping("page")
    public Object findGoods(@RequestBody SearchRequest searchRequest){
        //去es索引库中查询
        NativeSearchQueryBuilder builder = new NativeSearchQueryBuilder();
        //构造条件 从es中的all字段中搜索
        builder.withQuery(QueryBuilders.matchQuery("all",searchRequest.getKey()).operator(Operator.AND));
        //过滤filter查询条件
        BoolQueryBuilder builder1 = QueryBuilders.boolQuery().must(QueryBuilders.matchQuery("all", searchRequest.getKey()));
        ...
        if (categoryList.size()==1){
            List<SpecParam> specParams = specClient.findParamsByCidAndSearching(categoryList.get(0).getId());
        ...
        //执行查询 totalelement总条数 totalPage总页数 content数据内容
